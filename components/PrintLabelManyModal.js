...
const PrintLabelManyModal = ({ isOpen, setIsOpen, dataId }) => {
  ...
  const getDetailLabel = useCallback(() => {
    const id = dataId;
    const arr = id.map((item) =>
      HttpRequestExternal.getDetailLabelManagement(item)
    );
    setIsLoading(true);
                ...
                <div className='mt-4 space-y-4'>
                  <div ref={componentRef} className='flex flex-col'>
                    {labelDetails.map((item, index) => {
                      return (
                        <>
                          <div className='flex my-2 border-4 rounded-md border-primary-600 w-sreen print:m-2'>
                              ...
                              <div className='px-6 py-4'>
                                <dl className='space-y-1'>
                                  ...
                                  <div>
                                    ...
                                    <dd className='-mt-0.5 font-semibold text-gray-900'>
                                      {item?.data?.data_brand?.brand_name ||
                                        '-'}
                                    </dd>
                                  </div>
                                </dl>
...
export default PrintLabelManyModal;
