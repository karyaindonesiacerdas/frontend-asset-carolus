...
export default function App(props) {
    ...
    const loadData = useCallback(() => {
        setIsLoading(true);
        HttpRequestExternal.listUnit().then((response) => {
            setListData(response.data.data)
            // // console.log("data list unit", response.data.data)
            setIsLoading(false);
        }).catch((error) => {
            // console.log(error, error.response);
            setListData([]);
            setIsLoading(false);
            notifyGagal("to load data")
        });
    }, [listData])
    ...
    const deleteData = useCallback((id) => {
        ...
        HttpRequestExternal.deleteUnit(id).then((response) => {
            refreshData()
            notifyDel()
