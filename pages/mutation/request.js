...
export default function App(props) {
  ...
  const getData = useCallback(() => {
    setIsloading(true);
    HttpRequestExternal.getDetailBooking(router.query.id)
      .then((res) => {
        ...
        setValue(
          "physical_check_bfr_booking",
          data?.physical_check_bfr_booking
        );
  ...
  const onSubmit = (value) => {
    ...
    HttpRequestExternal.approveBooking(router.query.id, data)
      .then((res) => {
        ...
        <div className="px-6 my-3 bg-white rounded-md sm:px-6">
          ...
          <div className="grid grid-cols-2">
            <div className="grid-cols-1 gird">
              <div>
                <h1 className="py-2 text-sm antialiased font-bold">
                  Asset Name :{" "}
                </h1>
              </div>
              <div>
                <h1 className="text-sm antialiased ">
                  {getDetail?.data_asset?.equipments_name ?? "-"}
                </h1>
              </div>
              <div>
              ...
              <div>
                <h1 className="text-sm antialiased">
                  {getDetail?.data_asset?.data_brand?.brand_name ?? "-"}
                </h1>
              </div>
              <div>
                <h1 className="py-2 text-sm antialiased font-bold">
              ...
              <div>
                <h1 className="text-sm antialiased">
                  {getDetail?.data_asset?.serial_number ?? "-"}
                </h1>
              </div>
              <div>
              ...
              <div>
                <h1 className="text-sm antialiased">
                  {getDetail?.data_asset?.data_model?.model_name ?? "-"}
                </h1>
              </div>
              <div>
              ...
              <div>
                <h1 className="text-sm antialiased">
                  {getDetail?.data_asset?.data_type?.type_name ?? "-"}
                </h1>
              </div>
            </div>

            <div>
              <img src={getImage} className="w-auto h-auto" />
            </div>
          </div>
          ...
          <div className="grid w-screen grid-cols-2 gap-x-2">
            ...
            <div>
              <h1 className="text-sm antialiased">
                {getDetail?.start_date_format ?? " - "} -{" "}
                {getDetail?.end_date_format ?? " - "}
              </h1>
            </div>
            <div>
              <h1 className="text-sm antialiased">
                {getDetail?.data_floor?.name ?? " - "}
              </h1>
            </div>
            ...
            <div>
              <h1 className="text-sm antialiased">
                {getDetail?.data_department?.dep_name ?? " - "}
              </h1>
            </div>
            <div>
              <h1 className="text-sm antialiased">
                {getDetail?.data_building?.name ?? " - "}
              </h1>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            {canEdit &&
              getDetail?.status_booking != 3 &&
              getDetail?.status_booking != 2 && (
